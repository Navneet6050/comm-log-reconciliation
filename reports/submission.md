...
| 1 | Numerical | Exclude ineligible campaign | **26** | **Reason**: Excluded 4 successfully delivered attempts (`delivery_status = 900`) from Campaign 9004.<br>**Evidence**: Campaign 9004 is `approval_awaiting`. Because it is not finalized (`approved`, `aborted`, `resumed`, `stopped`), it is ineligible, and its delivered attempts must be excluded. |
